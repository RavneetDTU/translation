...
logfile = 'translation_controller.py'
...
class TranslationController:
    ...
    def __translate(self, input_text, input_language, output_language):
        ...
        if translation_handler is None:
            output_text, error = self.__translate_using_intermediate_handler(input_text,
                                                                             input_language,
                                                                             output_language)
            logging.error(logfile+' error in __translate function')
        else:
    ...
    def get_translation(self, translate_data, user):
        ...
        if language_helper is None:
            logging.error(logfile+' error in get_translation function, Invalid output language%s' % translation_dict["output_language"])
        ...
        if translation_dict.get("input_language"):
            ...
            if language_helper is None:
                logging.error(logfile+' error in get_translation function, Invalid input language %s' % translation_dict["input_language"])
        ...
        if translation_dict.get("input_language", None) is None:
            ...
            language_helper = self.__get_language_helper(translation_dict["input_language"])
            if language_helper is None:
                logging.error(logfile+' error in get_translation function, Invalid input language %s detected' % translation_dict["input_language"])
        ...
        if error:
            logging.error(logfile+' error in get_translation function, line no 54 i.e. %s' % error )
            return None, error
    ...
    def __translate_using_intermediate_handler(self, input_text, input_language, output_language):
        ...
        logging.error(logfile+' Cannot translate from %s to %s' % (input_language, output_language))
    ...
    def add_suggestion(self, suggestion, user_data):
        ...
        if translation is None:
            logging.error(logfile+' error in add_suggestion function, Invalid Translation ID %s' % suggestion.translation_id)
        ...
        db_session.flush()
        logging.info(logfile+' add_suggestion function worked perfectly fine')
        return None
    ...
    def __get_translation_by_id(self, translation_id: int):
        ...
        logging.info(logfile+' __get_translation_by_id function is working fine')
        return translation
    ...
    def __get_all_translation_by_input_output_language(self, input_language, output_language, start_time, end_time):
        ...
        logging.info(logfile+' __get_all_translation_by_input_output_language is working fine')
        return translations
    ...
    def __get_all_translation_by_user_id(self, user_id, start_time, end_time):
        ...
        logging.info(logfile+' __get_all_translation_by_user_id is working fine')
        return translations

    def get_translated_characters_by_input_output_language(self, input_language, output_language, start_time, end_time):
        if start_time > end_time:
            logging.info(logfile+' Start time must be less than end time')
            return None, "Start time must be less than end time"
        language_helper = self.__get_language_helper(output_language)
        if language_helper is None:
            logging.error(logfile+' Invalid output language %s in get_translated_characters_by_input_output_language' % output_language)
            return None, "Invalid output language %s" % output_language
        language_helper = self.__get_language_helper(input_language)
        if language_helper is None:
            logging.error(logfile+' Invalid input language %s in get_translated_characters_by_input_output_language' % input_language)
        ...
        if not translations:
            logging.info(logfile+' Translation not found with input language %s and output language %s in requested time range' % (
    ...
    def get_translated_characters_by_user_id(self, user_id, start_time, end_time):
        ...
        if not user:
            logging.error(logfile+' Invalid user id in get_translated_characters_by_user_id function  %s' % user_id)
    ...
    def get_model_status(self, input_language, output_language):
        language_helper = self.__get_language_helper(output_language)
        if language_helper is None:
            logging.error(logfile+' Invalid output language error in get_model_status function  %s' % output_language)
            return None, "Invalid output language %s" % output_language
        language_helper = self.__get_language_helper(input_language)
        if language_helper is None:
            logging.error(logfile+' Invalid input language error in get_model_status function %s' % input_language)
            return None, "Invalid input language %s" % input_language
